...
This project is being compiled using [Quarto](https://quarto.org) in [RStudio](https://posit.co/downloads/). Not only does Quarto allow for professional and polished looking content; Quarto, RStudio, and [R](https://www.r-project.org/) are "free". There is a learning curve, so opportunity costs will apply. Another advantage of Quarto is the ability to embed and share code chunks in the final product. To that end: I will walk through the processes, step by step.

The entire project including all downloaded *.csv files are available [on GitHub](https://github.com/dasneddon/local_bias).
...
I have created a function called *gravitymap()* for generating the map visualizations in the appendix.
```{r}
...
names(reg7$coefficients) <- coefnames
...
7. Canada and USA robust standard errors \
...
```{r}
predictions2 <- predict(reg2, newdata=us_ca)
predictions4 <- predict(reg4, newdata=us_ca)
predictions5 <- predict(reg5, newdata=us_ca)   
predictions6 <- predict(reg6, newdata=us_ca)
predictions7 <- predict(reg7, newdata=us_ca)
predictions3 <- predict(reg3, newdata=us_ca[(us_ca$from_gdp > 10^4) & (us_ca$to_gdp > 10^4),])

residuals2 <- log(us_ca$value) - predictions2
residuals3 <- log(us_ca$value) - predictions3
residuals4 <- log(us_ca$value) - predictions4
residuals5 <- log(us_ca$value) - predictions5
residuals6 <- log(us_ca$value) - predictions6
residuals7 <- log(us_ca$value) - predictions7

mae2 <- mean(abs(residuals2))
mae3 <- mean(abs(residuals3))
mae4 <- mean(abs(residuals4))
mae5 <- mean(abs(residuals5))
mae6 <- mean(abs(residuals6))
mae7 <- mean(abs(residuals7))

mae <- data.frame(mae2, mae3, mae4, mae5, mae6, mae7)
...
![Alberta](maps/AB.png)
![British Columbia](maps/BC.png)
![Manitoba](maps/MB.png)
![New Brunswick](maps/NB.png)
![Newfoundland](maps/NL.png)
![Nova Scotia](maps/NS.png)
![Ontario](maps/ON.png)
![Prince Edward Island](maps/PE.png)
![Quebec](maps/QC.png)
![Saskatchewan](maps/SK.png)
